...
const MAX_WORKFLOW_FILES_TO_INSPECT = 10;
const GITHUB_FETCH_TIMEOUT_MS = 6_000;
const LARGE_TREE_FETCH_TIMEOUT_MS = 10_000;
...
export async function analyzeRepo(owner: string, repo: string) {
  ...
  async function getRepositoryTree(): Promise<GitTreeItem[]> {
    /*
     * First try GitHub's recursive tree even for large repositories.
     * The response is far more accurate for monorepos such as React and Next.js.
     * If GitHub times out, truncates heavily, or rejects the request, fall back
     * to a smaller signal-focused scan instead of failing the whole report.
     */
    try {
      const response = await githubFetch(
      ...
      const usefulTree = selectUsefulTreeItems(data.tree);

      if (usefulTree.length > 0) {
        return usefulTree.slice(0, MAX_TREE_ITEMS);
      }
  ...
  function selectUsefulTreeItems(tree: GitTreeItem[]): GitTreeItem[] {
    const useful = tree.filter((item) => {
      ...
      // Always keep root-level files/configuration.
      if (depth <= 2) return true;

      // Keep paths that provide strong RepoDoctor signals.
      return (
        /(^|\/)(__tests__|tests?|spec)(\/|$)/i.test(path) ||
        /\.(test|spec)\.(js|jsx|ts|tsx|mjs|cjs|py|php|rb|go|java|kt|cs)$/i.test(path) ||
        /(^|\/)(package\.json|tsconfig(\.[^/]+)?\.json|jest\.config\.[^/]+|vitest\.config\.[^/]+|playwright\.config\.[^/]+|cypress\.config\.[^/]+)$/i.test(path) ||
        /(^|\/)(eslint\.config\.[^/]+|\.eslintrc(\.[^/]+)?|prettier\.config\.[^/]+|\.prettierrc(\.[^/]+)?|\.editorconfig)$/i.test(path) ||
        /(^|\/)(package-lock\.json|pnpm-lock\.yaml|yarn\.lock|bun\.lockb?|poetry\.lock|pipfile\.lock|composer\.lock|gemfile\.lock|cargo\.lock)$/i.test(path) ||
        /(^|\/)(dockerfile(\.[^/]+)?|\.env\.(example|sample|template))$/i.test(path) ||
        /^\.github\//i.test(path)
      );
    });
  ...
  async function getLimitedRepositoryTree(): Promise<GitTreeItem[]> {
    try {
      ...
      /*
       * For large monorepos, recursively inspect a small set of high-value
       * directories. This preserves test/config detection without scanning
       * the entire repository when the main recursive tree request fails.
       */
      ...
      const directoriesToInspect = rootTree
        .filter(
          (item) =>
            item.type === "tree" &&
            item.path &&
        ...
        directoriesToInspect.map(async (directory) => {
          try {
            const directoryResponse = await githubFetch(
              `${base}/git/trees/${directory.sha}?recursive=1`,
              undefined,
              LARGE_TREE_FETCH_TIMEOUT_MS
            );
            ...
            return selectUsefulTreeItems(
              directoryData.tree.map((item) => ({
                ...item,
                path:
  ...
  async function analyzeWorkflows(): Promise<WorkflowAnalysis> {
    try {
      ...
      const workflowNames = workflowFiles
        ...
        .join("\n")
        .toLowerCase();
      const searchableWorkflowText = `${workflowNames}\n${combined}`;
      ...
      return {
        ...
        hasBuild:
          /\bbuild\b/.test(searchableWorkflowText) ||
          /npm run build|pnpm build|yarn build|dotnet build|mvn .*package|gradle .*build/.test(searchableWorkflowText),
        hasTest:
          /\btest(s|ing)?\b/.test(searchableWorkflowText) ||
          /npm test|npm run test|pnpm test|yarn test|pytest|vitest|jest|dotnet test|mvn .*test|gradle .*test/.test(searchableWorkflowText),
        hasLint:
          /\blint\b/.test(searchableWorkflowText) ||
          /eslint|ruff|flake8|pylint|golangci-lint/.test(searchableWorkflowText),
        hasTypeCheck:
          /typecheck|type-check|tsc --noemit|mypy|pyright/.test(searchableWorkflowText),
        hasDeploy:
          /\bdeploy\b/.test(searchableWorkflowText) ||
          /vercel|netlify|pages|firebase deploy|aws-actions|azure\/webapps-deploy|google-github-actions\/deploy/.test(searchableWorkflowText),
        hasSecurity:
          /codeql|security|snyk|trivy|semgrep|dependency-review-action/.test(searchableWorkflowText),
        hasRelease:
          /\brelease\b/.test(searchableWorkflowText) ||
          /softprops\/action-gh-release|semantic-release|changesets\/action/.test(searchableWorkflowText),
      };
    } catch {
      return emptyWorkflowAnalysis();
    }
